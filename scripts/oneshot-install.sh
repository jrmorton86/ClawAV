...
#   curl -sSL https://raw.githubusercontent.com/coltz108/ClawAV/main/scripts/oneshot-install.sh | sudo bash -s -- --version v0.2.0
...
if [[ -n "$AGENT_USERNAME" ]]; then
    log "Locking down agent account '$AGENT_USERNAME' with sudoers allowlist..."
    ...
    POLICY_FILE="/etc/sudoers.d/010_pi-nopasswd"
...
# This file is immutable (chattr +i) — requires admin to modify.

# ── ALLOW RULES (base grants) ──

# Read-only tools
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/cat, /usr/bin/ls, /usr/bin/head, \\
...
# Log & diagnostics
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/journalctl, /usr/bin/dmesg, \\
...
# Service management (start/restart only — stop/disable/mask denied below)
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/systemctl status *, \\
...
# Package management
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/apt, /usr/bin/apt-get, \\
    /usr/bin/dpkg, /usr/bin/apt-cache

# Write tools (restricted by deny rules below)
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/tee, /usr/bin/tee -a *, \\
...
$AGENT_USERNAME ALL=(ALL) NOPASSWD: /usr/bin/sed, /usr/bin/awk, \\
    /usr/bin/curl, /usr/bin/wget

# ── DENY RULES (last match wins — override allows above) ──

# Shells & interpreters (root shell equivalent)
$AGENT_USERNAME ALL=(ALL) !/usr/bin/bash, !/usr/bin/sh, !/usr/bin/dash, \\
...
# su / sudo escalation
$AGENT_USERNAME ALL=(ALL) !/usr/bin/su, !/usr/bin/su *, \\
    !/usr/sbin/su, !/usr/sbin/su *

# User/password management
$AGENT_USERNAME ALL=(ALL) !/usr/bin/passwd, !/usr/bin/passwd *, \\
    !/usr/bin/chpasswd, \\
...
# Sudoers editing
$AGENT_USERNAME ALL=(ALL) !/usr/sbin/visudo, !/usr/bin/sudoedit

# Process killing
$AGENT_USERNAME ALL=(ALL) !/usr/bin/kill, !/usr/bin/kill *, \\
    !/usr/bin/killall, !/usr/bin/killall *, \\
    !/usr/bin/pkill, !/usr/bin/pkill *

# systemctl destructive ops
$AGENT_USERNAME ALL=(ALL) !/usr/bin/systemctl stop *, \\
    !/usr/bin/systemctl disable *, \\
    !/usr/bin/systemctl mask *

# chattr (prevent immutability manipulation)
$AGENT_USERNAME ALL=(ALL) !/usr/bin/chattr, !/usr/bin/chattr *

# In-place file editing & raw writes
$AGENT_USERNAME ALL=(ALL) !/usr/bin/sed -i *, !/usr/bin/sed --in-place *
$AGENT_USERNAME ALL=(ALL) !/usr/bin/dd, !/usr/bin/dd *

# Write tools targeting auth files (/etc/shadow, passwd, group, gshadow)
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/shadow, !/usr/bin/tee -a /etc/shadow, \\
...
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/sudoers, !/usr/bin/tee -a /etc/sudoers, \\
...
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/ssh/*, !/usr/bin/tee -a /etc/ssh/*, \\
...
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/pam.d/*, !/usr/bin/tee -a /etc/pam.d/*, \\
    !/usr/bin/cp * /etc/pam.d/*, !/usr/bin/mv * /etc/pam.d/*, \\
    !/usr/bin/rm /etc/pam.d/*, !/usr/bin/rm -f /etc/pam.d/*

# Write tools targeting ClawAV
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /usr/local/bin/clawav, \\
...
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/ld.so.preload, !/usr/bin/tee -a /etc/ld.so.preload, \\
...
$AGENT_USERNAME ALL=(ALL) !/usr/bin/tee /etc/cron.d/*, !/usr/bin/tee -a /etc/cron.d/*, \\
...
POLICYEOF
    ...
    if visudo -cf "$POLICY_FILE"; then
        ...
        log "✓ Agent account locked down with allowlist policy"
    else
        # Restore basic NOPASSWD so we don't brick the agent
        echo "$AGENT_USERNAME ALL=(ALL) NOPASSWD: ALL" > "$POLICY_FILE"
        chmod 440 "$POLICY_FILE"
        warn "Sudoers allowlist had syntax errors — fell back to NOPASSWD:ALL. Agent not locked down!"
    fi
fi
